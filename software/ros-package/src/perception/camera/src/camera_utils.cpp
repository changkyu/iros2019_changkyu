...
{
    const vector<float> &workspace = caminfo.workspace;
    const vector<float> &camera_RT = caminfo.camera_RT;
                        ...
                        const bool crop )
{
    ...
    utils::PointCloudfromDepth<PointXYZRGB, uint16_t>(
            ...
            caminfo.camera_K, vector<float>(), image, cv::Mat() );
